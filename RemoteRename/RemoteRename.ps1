...
$lessThan16 = $true
$16response = "y"
#default variables regarding the target name being above 16 characters
...
if ($nombre.length -gt 16){
	$lessThan16 = $false
	Write-Host "WARNING: `nNew host name of target PC is longer than 16 characters. This can cause issues with Azure AD, SQL, etc.." -ForegroundColor Red
	$16response = Read-Host "Would you like to continue with the process? (CAN CAUSE ISSUES) [y/n]" -ForegroundColor Yellow
}

if ($16response -eq "n"){
	$nombre = Read-Host "Please enter new host name of target PC [16>= characters]"
}
